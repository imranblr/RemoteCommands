...
class node(object):
    def __init__(self, hostname=None, port=None, username=None, password=None, keyfile=None, timeout=10):
            ...
            if self.keyfile is not None:
                ...
                if os.path.exists(self.keyfile):
                    client.connect(self.hostname, port=self.port,
                                   username=self.username, key_filename=self.keyfile, timeout=self.timeout)
    ...
    def ExecCommand(self, command, RequiresRoot=False, timeout=150):
        if not self.__isconnected:
            if not self.Connect():
                return None
        if RequiresRoot:
